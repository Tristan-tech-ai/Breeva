...
export default function LoginPage() {
  ...
  const strengthLabel = ['', 'Weak', 'Weak', 'Fair', 'Strong', 'Very Strong'][strengthScore] || '';
  const strengthColor = ['', 'bg-red-500', 'bg-orange-500', 'bg-amber-500', 'bg-primary-500', 'bg-emerald-500'][strengthScore] || '';
  const strengthTextColor = ['', 'text-red-500', 'text-orange-500', 'text-amber-500', 'text-primary-500', 'text-emerald-500'][strengthScore] || '';
                          ...
                          <div className="mt-2">
                              ...
                              {[1, 2, 3, 4, 5].map((level) => (
                                <div
                                  ...
                                  className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                                    level <= strengthScore ? strengthColor : 'bg-gray-200 dark:bg-gray-700'
                                  }`}
                                />
                              ))}
                            </div>
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2 text-xs text-gray-400">
                                <span className={`flex items-center gap-0.5 ${password.length >= 8 ? 'text-emerald-500' : ''}`}>
                                  {password.length >= 8 ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />} 8+
                                </span>
                                <span className={`flex items-center gap-0.5 ${/[A-Z]/.test(password) ? 'text-emerald-500' : ''}`}>
                                  {/[A-Z]/.test(password) ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />} A-Z
                                </span>
                                <span className={`flex items-center gap-0.5 ${/[0-9]/.test(password) ? 'text-emerald-500' : ''}`}>
                                  {/[0-9]/.test(password) ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />} 0-9
                                </span>
                                <span className={`flex items-center gap-0.5 ${/[^A-Za-z0-9]/.test(password) ? 'text-emerald-500' : ''}`}>
                          ...
                          >
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                placeholder="••••••••"
                                className={`w-full pl-11 pr-12 py-3 rounded-xl border bg-white/50 dark:bg-gray-800/50 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500/30 transition ${
